# DIGRA — churn-maintenance benchmark fork
...
| `hnsw_churn.cpp`  | same workload on **vanilla hnswlib** (`addPoint`/`markDelete`) — the pure-vector CPU dynamic baseline, and a sanity check on the DIGRA repro. Also records `cur_element_count` per round (live + accumulated tombstones) for the memory-footprint comparison |
| `hnsw_reclaim_probe.cpp` | small probe proving hnswlib's `markDelete` never reclaims slots: under churn, `cur_element_count` climbs monotonically (live stays flat, tombstones accumulate) until it hits `max_elements` |
| `CMakeLists.txt`  | builds all three into `bin/`; **no forced `-mavx512f`** (see Gotchas) |
| `.gitignore`      | ignores `bin/`, CMake artifacts, `results/`, `*.csv`, the `data` symlink |

All three binaries build into `bin/` with a single `make`.
...
`round,maintenance_ms,recall,live,used_slabs`.

### Memory-footprint probe (does HNSW reclaim deleted slots?)

```bash
./bin/hnsw_reclaim_probe
```

Builds a small index, then runs churn rounds and prints `cur_element_count`,
`deleted`, `live`, and `max_elements` each round. Result: `cur_element_count`
climbs monotonically (e.g. 1000 -> 3000 over 10 rounds) while `live` stays flat
at 1000 and `deleted` climbs 0 -> 2000, until it hits `max_elements`. This
proves hnswlib's `markDelete` is a pure tombstone — it never reclaims, so
memory grows without bound under sustained churn and the index must eventually
be rebuilt. (ETALE, by contrast, reclaims deleted-node slabs via a periodic
compact+reclaim pass, keeping its footprint bounded.)
